...
export default function FormValidator(method) {
  switch (method) {
    case "createProject": {
      return [
        body("title", "Project title missing")
          .exists()
          .isLength({ min: 6 }),
    ...
    case "signup": {
      return [
        body("user.username", "Invalid username")
          .exists()
          .isLength({ min: 6 }),
        body("user.email", "Invalid email")
          .exists()
          .isEmail(),
        body("user.password", "Invalid password")
          .exists()
          .isLength({ min: 6 }),
